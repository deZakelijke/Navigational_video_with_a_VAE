...
class ConvLSTMCell(nn.Module):
    ...
    def inital_state(self, n_samples):
        hidden_state = torch.zeros((n_samples, self._out_channels, self.size_y, self.size_x))
        cell_state = torch.zeros((n_samples, self._out_channels, self.size_y, self.size_x))
...
class ConvLSTMPositiontoImageDecoder(nn.Module):
    ...
    def forward(self, poses):
        ...
        canvas = torch.zeros((len(poses), self.n_canvas_channels, self.n_y_canvas, self.n_x_canvas))
...
class ConvLSTMImageToPositionEncoder(nn.Module):
    ...
    def forward(self, image):
        ...
        return torch.distributions.Normal(loc=mu, scale=ff.softplus(logsig))
